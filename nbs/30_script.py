# %% [markdown]
# # Experiment 30: 30_testing_losses_lung_spleen copy
# 
# 'Fork' of 29.
# 
# Average Test Dice: 
...
# %%
EXP_NAME = "30_testing_losses_lung_spleen copy"
...
# %%
organs_of_interest = "lung spleen".split()
losses = {
    "tversky_1_9": monai.losses.TverskyLoss(softmax=True, alpha=0.1, beta=0.9),
    "tversky_3_7": monai.losses.TverskyLoss(softmax=True, alpha=0.333, beta=0.667),
    "tversky_7_3": monai.losses.TverskyLoss(softmax=True, alpha=0.667, beta=0.333),
    "tversky_9_1": monai.losses.TverskyLoss(softmax=True, alpha=0.9, beta=0.1),
}

# %%
metrics_log = {"organ": [], "loss":[], "threshold":[], "train_dice": [], "valid_dice": [], "test_dice": []}
...
for organ in TRAIN_DF.organ.unique():
    for loss_name, criterion in losses.items():
        ...
        runner.train(
            ...
            num_epochs=EPOCHS,
            callbacks=callbacks,
            logdir=LOG_DIR/f"{organ}_{loss_name}",
            valid_loader="valid",
            valid_metric="loss",
        ...
        metrics_log["loss"].append(loss_name)

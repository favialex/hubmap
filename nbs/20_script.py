...
for organ in TRAIN_DF.organ.unique():
    ...
    ds_size = int(len(ADD_TRAIN) % BATCH_SIZE)
    if ds_size > 0: ADD_TRAIN = ADD_TRAIN[:-ds_size]
    ds_size = int(len(ADD_VALID) % BATCH_SIZE)
    if ds_size > 0: ADD_VALID = ADD_VALID[:-ds_size]

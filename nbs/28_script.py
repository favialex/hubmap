# %% [markdown]
# # Experiment 28: 28_deeplabv3plus
# 
# Average Test Dice: 
# 
...
class MonaiSupervisedRunner(catalyst.dl.SupervisedRunner):
    def forward(self, batch):
        if self.is_train_loader:
            if batch[self._input_key].shape[0] == 1: self.model.eval() # DeepLabV3Plus compatibility
...
for organ in TRAIN_DF.organ.unique():
    ...
    if organ != "prostate":
        ...
        model = load_weights(model, "../logs/28_deeplabv3plus/prostate_denoise/checkpoints/model.best.pth")
        model = model.to(DEVICE)
    ...
    model.segmentation_head[0] = torch.nn.Conv2d(256, 2, kernel_size=(1, 1), stride=(1, 1))
    model = model.to(DEVICE)
